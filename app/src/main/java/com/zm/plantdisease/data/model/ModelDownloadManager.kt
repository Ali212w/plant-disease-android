...
/**
 * Downloads and manages the local model files used by the app.
 * The main default model keeps the original release name: model.ptl.
 */
object ModelDownloadManager {

    const val MODEL_FILE_NAME = "model.ptl"
    const val MODEL_A_FILE_NAME = "model_A.ptl"
    const val MODEL_B_FILE_NAME = "model_B.ptl"
    const val BASELINE_FILE_NAME = "baseline.ptl"
    ...
    fun downloadFile(
        ...
        Thread {
            try {
                ...
                if (tempFile.renameTo(localFile)) onSuccess() else onError("فشل في حفظ النموذج")
            } catch (e: Exception) {
                tempFile.delete()
